...
# Fixed CSS for visibility - ensuring high contrast and inheriting theme colors
st.markdown("""
    <style>
    /* Metric Card Styling */
...
if any(x is None for x in [p_df, s_df, t_df, v_df]):
    st.error("Required data feeds not detected. Please check your folder structure.")
    st.stop()
...
if "Outside Occupancy" in scenario:
    ...
    if "Manager" in view_mode:
        st.info("This condition resulted in avoidable HVAC energy cost during non-operational hours.")
        st.metric("Primary Impact: Estimated Cost Loss", f"₹{int(waste_df['Energy_Cost'].sum()):,}")
        ...
        explainability_manager("HVAC Operating Outside Occupancy Hours", 
...
# =====================================================================
    ...
    # ===== MANAGER VIEW =====
    if "Manager" in view_mode:
        st.info("This condition indicates inefficient cooling control leading to continuous energy waste.")
        st.metric("Primary Impact: Estimated Cooling Loss", f"₹{int(df['Cooling_Loss'].sum()):,}")

        cols = st.columns(3)
        cols[0].metric("Projected Monthly Loss", f"₹{int(df['Cooling_Loss'].sum()*30):,}")
        cols[1].metric("Avg Temp Deviation", f"{df[df['Fault']]['Delta_T'].mean():.1f}°C" if df["Fault"].any() else "0.0°C")
        cols[2].metric("CO₂ Impact", f"{int(calculate_carbon(df['Cooling_Loss'].sum()/15))} kg")

        st.line_chart(df[["Room_Temp", "Setpoint"]])
        explainability_manager("Excess Cooling / Control Inefficiency", 
    ...
    # ===== OPERATOR VIEW =====
        ...
        explainability_operator(["Temp < setpoint", "Valve > 80%", "Sustained condition"], 
